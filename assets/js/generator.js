...
const Generator = {
  ...
  async build(config) {
    ...
    const sqlFiles = [
      ...
      // Keep upgrade artifacts complete: existing installations may need these
      // idempotent releases even though fresh installs use complete-schema.sql.
      'database/update-v13-schema.sql',
      'database/update-v14-schema.sql',
      'database/update-v15-schema.sql',
      'database/update-v16-security-and-navigation.sql',
    ...
    const dedicatedPages = [
      ...
      { id: 'school_fees',        name: 'School Fee Structure', fn: () => staticPages['school-fees'] || T.modulePage(resolvedConfig, 'school_fees') },
      { id: 'school-products',    name: 'School Products',      fn: () => staticPages['school-products'] || T.modulePage(resolvedConfig, 'school_products') },
      { id: 'school_products',    name: 'School Products',      fn: () => staticPages['school-products'] || T.modulePage(resolvedConfig, 'school_products') },
      { id: 'status-manager',     name: 'Role & Status Manager', fn: () => staticPages['status-manager'] || T.modulePage(resolvedConfig, 'status_manager') },
      { id: 'status_manager',     name: 'Role & Status Manager', fn: () => staticPages['status-manager'] || T.modulePage(resolvedConfig, 'status_manager') },
      ...
      { id: 'hmg_digital_products', name: 'HMG Digital Products', fn: () => staticPages['hmg-digital-products'] || T.modulePage(resolvedConfig, 'hmg_digital_products', { requireRole: 'all' }) },
    ];
    ...
    for (const modId of selectedModules) {
      ...
      const aliases = {
        ...
        feature_guide: 'feature-guide', school_fees: 'school-fees', school_products: 'school-products', status_manager: 'status-manager', hmg_digital_products: 'hmg-digital-products', ecosystem_products: 'ecosystem-products'
      };
