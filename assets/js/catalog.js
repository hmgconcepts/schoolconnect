...
const PRESETS = {
  ...
  'secondary': {
    ...
    modules: ['academic_setup','students','staff','classes','subjects','attendance','punctuality','cbt-prompts','entrance','results','certificates','flyer','report-cards','analytics','admin-data','academic_records','storage','approvals','timetable-generator','checkin','diary','surveys','menu','settings','fees','timetable','exam_timetable','cbt','sow','messages','announcements','events','gallery','library','digital_library','assignments','parents','idcards','directory','departments','broadcast','complaints','leave','visitors','developer'],
    levels:['JSS 1','JSS 2','JSS 3','SSS 1','SSS 2','SSS 3']
  },
  'fullschool': {
    name:'Group of Schools (K–12)', emoji:'🏢',
    modules: ['academic_setup','students','staff','classes','subjects','attendance','punctuality','cbt-prompts','entrance','results','flyer','report-cards','analytics','admin-data','academic_records','storage','approvals','timetable-generator','checkin','diary','surveys','menu','settings','fees','timetable','exam_timetable','cbt','sow','messages','announcements','events','gallery','library','digital_library','assignments','parents','idcards','directory','departments','broadcast','complaints','leave','visitors','hostel','transport','alumni','certificates','admissions','promotion','finance','inventory','payroll','staff_loans','staff_bonus','appraisals','rubrics','transcripts','transfer_cert','counselling','voting','health','conduct','eresources','birthdays','lms','gamification','cafeteria','financial_aid','front_desk','career_counseling','document_builder','fleet_tracking','facility_booking','compliance','developer'],
...
const MODULES = [
  ...
  { id:'exam_timetable', name:'Exam Timetable',          group:'Core',  desc:'Build and publish the examination schedule: papers, dates, times, venues, invigilators. Clash detection + one-click auto-spread of all subjects across a date range.', new:true },
  { id:'sow',          name:'Scheme of Work',          group:'Core',  desc:'Termly planning + weekly progress tracking with proprietor dashboard.', new:true },
  { id:'cbt',          name:'CBT / Online Exams',      group:'Core',  desc:'V5.1 server-verified CBT: legacy answer-key diagnosis/repair, pre-publish validation, CSV upload, UTME tabs, timer, original-index randomisation, negative marking, anti-cheat, certificates and report-card mapping. Missing keys fail loudly—never a silent false zero.', popular:true },
  ...
  /* V9.9 (#3): 'hr' merged into 'payroll' — one Payroll Register page, no duplicates. The id stays resolvable (generator maps hr→payroll.html) so existing configs still build. */
...
const CHATBOT_KB = [
  ...
  { match: ['deploy','publish','host','github','live'], reply: 'Deployment is 100% free. After unzipping:\n1. Deploy to **Vercel** or **Netlify** (free tier)\n2. Upload all files or connect to Git repository\n3. Add **Supabase** keys to `assets/js/config.js`\n4. Run only `database/complete-schema.sql` once in the Supabase SQL editor.\n\nSee **DEPLOYMENT-GUIDE.md** for details.' },
  ...
  { match: ['cost','price','free','pay'], reply: 'This is a proprietary licensed platform developed and maintained by HMG Technologies. Enterprise-grade infrastructure ensures reliability and security.' },
  { match: ['feature','module','what'], reply: 'You get up to **97 modules** covering every aspect of school management: Academics, CBT, Finance, HR, Payroll, Communication, and more. See **FEATURES.md**.' },
...
console.log('%c[School Connect Gen v3] catalog loaded — 86 themes, 45 fonts, 7 presets, 97 modules (CBT engine + super features: chatbot, command palette, ID cards, certificates, flyer).', 'color:#4f46e5;font-weight:bold;font-size:13px');
