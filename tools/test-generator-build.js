...
async function build(kind){
 ...
 const zip=await JSZip.loadAsync(buf), names=Object.keys(zip.files); const must=['index.html','login.html','dashboard.html','cbt-exam.html','cbt-multi.html','report-cards.html','assets/js/cbt-engine.js','assets/js/report-engine.js','assets/js/data-portability.js','database/complete-schema.sql','database/cbt-v5.1-zero-score-hotfix.sql','database/cbt-v5.1.1-getter-school-settings-fix.sql','database/v5.3-platform-enhancements.sql','database/v5.4-portability-cbt-metrics.sql','database/demo-seed.sql','assets/img/logo.svg'];
 ...
 if(!cbt.includes('exam-school-logo')||!cbt.includes('renderTabs=function')||!cbt.includes("rpc('cbt_submit_v5'")||!cbt.includes('obsolete CBT grading engine'))throw new Error(kind+' has stale CBT template');
 if(!sql.includes('sc_cbt_answer_matches')||!sql.includes('cbt_submit_v5')||!sql.includes('engine_version'))throw new Error(kind+' has stale schema');
 const hotfix=await zip.file('database/cbt-v5.1-zero-score-hotfix.sql').async('string');if(!hotfix.includes("'answer_key_missing'")||!hotfix.includes('cbt_diagnose_exam'))throw new Error(kind+' has stale CBT hotfix');
