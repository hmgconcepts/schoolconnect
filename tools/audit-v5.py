...
ok('Timetable wizard: whole-school multi-class flow with mapped teachers and teacher grids',all(x in timetable for x in ['Admin Timetable Wizard','Step 1','Step 2','Step 3','Step 4','id="tt-classes"','id="tt-subject"','tt-teacher-auto','id="tt-term"','id="tt-session"','TTG.validate','TTG.printGrid','masterGrid','teacherGrids','pc-day-len','refreshPpdInfo'])and 'dash-my-timetable' in (ROOT/'dashboard.html').read_text())
...
ok('V9.1 timetable: double periods, early-period cap, teacher-availability always enforced','double_periods' in schema and 'max_period' in schema and (ROOT/'timetable-generator.html').read_text().count('tt-dbl')>=1 and (ROOT/'timetable-generator.html').read_text().count('tt-maxp')>=1)
ok('V9.1 exam timetable page + table shipped',(ROOT/'exam-timetable.html').exists() and 'exam_timetable' in schema and 'autoRun' in (ROOT/'exam-timetable.html').read_text() and 'clash' in (ROOT/'exam-timetable.html').read_text().lower())
ok('V9.1 subscription hardening: server verdict + activation keys + salt lock',all(x in schema for x in ['sc_license_status','sc_license_apply','sc_license_set_salt','sc_private.license_secret','license_events']) and 'fromServer' in (ROOT/'assets/js/license.js').read_text() and 'applyKey' in (ROOT/'assets/js/license.js').read_text() and (ROOT/'tools/make-license-key.mjs').exists())
ok('V9.1 optional BYO-key AI assistant (off by default, staff-only)',(ROOT/'assets/js/ai-assistant.js').exists() and 'sc_ai_settings' in schema and 'ai-enabled' in (ROOT/'settings.html').read_text())
ok('V9.1 assignments: term score sheet + quick physical assignment','AP.matrix' in (ROOT/'assignments.html').read_text() and 'quickAssignment' in (ROOT/'assignments.html').read_text() and 'exportMatrixCSV' in (ROOT/'assignments.html').read_text())
ok('V9.2 timetable access control: admin + authorized editors only',all(x in schema for x in ['sc_module_editors','sc_can_edit','tt_req_editor_write','examtt_write','tb_editor_write']) and 'tt-editors-card' in (ROOT/'timetable-generator.html').read_text() and 'resolveEdit' in (ROOT/'exam-timetable.html').read_text() and 'gateEdit' in (ROOT/'timetable.html').read_text())
ok('V9.2 exam auto-spread uses real subject sources (no subjects.class)',"select('name,class')" not in (ROOT/'exam-timetable.html').read_text() and 'timetable_requirements' in (ROOT/'exam-timetable.html').read_text())
_v92=schema[schema.rindex('EMBEDDED: database/v9.2-access-and-fixes.sql'):]
ok('V9.2 attendance: canonical family-scoped read policy (v9.2 embed drops all legacy + recreates one)',all(x in _v92 for x in ['drop policy if exists "att_read"','drop policy if exists "attendance_parent_read_v16"','drop policy if exists "attendance_scope_select"','create policy "v7_attendance_read_family"','guardian_email']) and _v92.count('on public.attendance for select')==1)
ok('V9.2 exam timetable on parent/student dashboards','dash-exam-tt' in (ROOT/'dashboard.html').read_text() and 'loadDashExamTT' in (ROOT/'assets/js/app.js').read_text() and 'dash-exam-tt' in (ROOT/'assets/js/templates.js').read_text())
ok('V9.2 wizard clarity: per-class ppw label + clear-demand + group doubles','per class' in (ROOT/'timetable-generator.html').read_text() and 'clearReqs' in (ROOT/'timetable-generator.html').read_text() and 'tt-group-dbl' in (ROOT/'timetable-generator.html').read_text() and 'tt-group-maxp' in (ROOT/'timetable-generator.html').read_text())
ok('V9.3 punctuality push uses dropdowns for term/session/class (no typing)','id=\"pp-term\" class=\"form-select\"' in (ROOT/'punctuality.html').read_text() and 'id=\"pp-sess\" class=\"form-select\"' in (ROOT/'punctuality.html').read_text() and 'id=\"pp-class\" class=\"form-select\"' in (ROOT/'punctuality.html').read_text() and 'academic_periods' in (ROOT/'punctuality.html').read_text())
ok('V9.3 staff GPS: multi-stage acquisition + retry (no 5s cold-fix timeout)','acquirePosition' in (ROOT/'checkin-staff.html').read_text() and 'watchPosition' in (ROOT/'checkin-staff.html').read_text() and 'ck-gps-retry' in (ROOT/'checkin-staff.html').read_text() and 'timeout: 5000' not in (ROOT/'checkin-staff.html').read_text() and 'timeout: 5000' not in (ROOT/'checkin.html').read_text())
ok('V9.4 fees: auto totals + fee-state RPC + smart fee form',all(x in schema for x in ['compute_class_fee_total','sc_student_fee_state','trg_compute_class_fee_total']) and 'sc_student_fee_state' in (ROOT/'assets/js/crud.js').read_text() and 'loadDashFees' in (ROOT/'assets/js/app.js').read_text() and 'dash-fees' in (ROOT/'dashboard.html').read_text() and 'FAM' in (ROOT/'fees.html').read_text())
ok('V9.4 promotion strip only on third-term report cards',"/third|3rd|\\b3\\b/i" in (ROOT/'assets/js/report-engine.js').read_text() and 'promotion?' in (ROOT/'assets/js/report-engine.js').read_text())
ok('V9.4 exam auto-spread: unlimited slots, paper divisions, pools, day-number mode',all(x in (ROOT/'exam-timetable.html').read_text() for x in ['exa-slots','exa-papers','exa-venues','exa-invs','exa-mode','day_no','toggleCompact']) and 'day_no' in schema and 'paper_kind' in schema)
ok('V9.4 timetable engine: two-phase double placement','PHASE 1: every subject' in schema and 'dbl_placed' in schema)
ok('V9.4 wizard: explicit multi-class targeting (no silent copies)','mcBanner' in (ROOT/'timetable-generator.html').read_text() and 'conscious decision' in (ROOT/'timetable-generator.html').read_text())
ok('V9.4 generator: social handles + sameAs SEO',all(x in (ROOT/'builder.html').read_text() for x in ['socialFacebook','socialWhatsApp','socialLinkedIn']) and 'sameAs' in (ROOT/'assets/js/generator.js').read_text() and 'socialBar' in (ROOT/'assets/js/generator.js').read_text())
ok('V9.4 drive-sync: urgency retries + one-tap heal + overdue banner',all(x in (ROOT/'assets/js/drive-sync.js').read_text() for x in ['overdueBanner','interactiveAskDay','visibilitychange']))
ok('V9.5 dashboard widgets fill ALL duplicate-id containers','querySelectorAll(\'[id="dash-fees"]\')' in (ROOT/'assets/js/app.js').read_text() and 'querySelectorAll(\'[id="dash-exam-tt"]\')' in (ROOT/'assets/js/app.js').read_text())
ok('V9.5 exam auto-spread: interleaved divisions + day pins + saturdays + shift + CSV',all(x in (ROOT/'exam-timetable.html').read_text() for x in ['exa-pins','exa-sat','shiftModal','exportCSV','INTERMIXED','todaySubjects']))
ok('V9.5 option group: own class tick-list + receipt popup',all(x in (ROOT/'timetable-generator.html').read_text() for x in ['tt-group-classes','tt-gc','gcBanner','Option group saved']))
ok('V9.5 wizard: engine-version check + post-generation audit + provenance badges',all(x in (ROOT/'timetable-generator.html').read_text() for x in ['engineStale','reconcile','removeGhosts','tt-audit','_addedIds','📦']) and "'engine','11'" in schema)
ok('V9.6+V10.1 leadership tier full access (owner cockpit + none-level excepted)','LEADERSHIP_OWNER_ONLY' in (ROOT/'assets/js/app.js').read_text() and 'PRINCIPAL_DENY:new Set([])' in (ROOT/'assets/js/app.js').read_text() and 'HEADTEACHER_DENY:new Set([])' in (ROOT/'assets/js/app.js').read_text() and "['bursar','principal','head_teacher','headteacher'].includes(r)){" in (ROOT/'assets/js/app.js').read_text() and 'LEADERSHIP_OWNER_ONLY.has(id))return false' in (ROOT/'assets/js/app.js').read_text())
ok('V9.6 CBT parseCSV accepts pasted CSV without a header row','looksHeader' in (ROOT/'assets/js/cbt-engine.js').read_text() and 'headerWords' in (ROOT/'assets/js/cbt-engine.js').read_text())
ok('V9.6 assignments: score button on every row (observer + ownership in modal)','injectScoreButtons' in (ROOT/'assignments.html').read_text() and 'MutationObserver' in (ROOT/'assignments.html').read_text())
ok('V9.7 leadership tier: full nav + page access + RLS (bursar in is_school_leader)','isLeader' in (ROOT/'assets/js/app.js').read_text() and "'principal','head_teacher','bursar'" in (ROOT/'assets/js/app.js').read_text() and "'head_teacher','bursar')and status" in schema and 'sc-cached-profile' in (ROOT/'assignments.html').read_text())
ok('V9.8 CBT prompts end with the required FILE sentence ×3 + deliverable + self-check',(ROOT/'cbt-prompts.html').read_text().count('Now output/generate the downloadable CSV file only.')==3 and (ROOT/'cbt-prompts.html').read_text().count('SELF-CHECK BEFORE OUTPUT')==3 and (ROOT/'cbt-prompts.html').read_text().count('DELIVERABLE: a single .csv FILE')==3)
ok('V9.8 window.App is exported (leadership write + admin buttons + logActivity all depend on it)','window.App = App;' in (ROOT/'assets/js/app.js').read_text())
ok('V9.8 assignments injector lists admin roles literally (belt-and-braces)',"'super_admin','superadmin','admin'" in (ROOT/'assignments.html').read_text())
ok('V9.8 settings fee card synchronised with class fee structure (itemised guard + shared-row note)','SYNC FIX' in (ROOT/'settings.html').read_text() and 'itemised' in (ROOT/'settings.html').read_text() and 'convert to a LUMP SUM' in (ROOT/'settings.html').read_text())
ok('V9.9 financial aid flows into fee state + dashboards','financial_aid' in schema and 'aid_rows' in schema and 'Scholarship / aid' in (ROOT/'assets/js/app.js').read_text() and 'Scholarship / aid' in (ROOT/'fees.html').read_text())
ok('V9.9 prompts: copy buttons on all five prompt cards',(ROOT/'cbt-prompts.html').read_text().count('CPUI.copy(')>=6)
ok('V9.9 HR merged into Payroll Register (redirect + no dead links)','payroll.html' in (ROOT/'hr.html').read_text() and 'location.replace' in (ROOT/'hr.html').read_text() and 'data-module-id="hr"' not in (ROOT/'dashboard.html').read_text())
...
ok('V10.1 attendance sync + report push',all(x in schema for x in ['sc_checkin_marks_attendance','sc_push_attendance_to_reports','trg_checkin_attendance']) and 'pushToReports' in (ROOT/'attendance.html').read_text())
ok('V10.1 SOW admin affirmation + lesson-plan vetting','admin_affirmed' in schema and 'review_feedback' in schema and 'needs-changes' in schema and 'admin_affirmed' in (ROOT/'assets/js/crud.js').read_text() and 'review_feedback' in (ROOT/'assets/js/crud.js').read_text())
ok('V10.1 master financial export (PDF+CSV)','sc_class_fee_master' in schema and 'fm-print-area' in (ROOT/'fees.html').read_text() and 'Master Financial Records' in (ROOT/'fees.html').read_text())
ok('V10.1 school shop on family dashboards','dash-products' in (ROOT/'dashboard.html').read_text() and 'loadDashProducts' in (ROOT/'assets/js/app.js').read_text())
ok('V10.1 module access: No-access level end-to-end',"'none'" in schema[schema.rindex('sc_module_access_leadership_check')-2000:schema.rindex('sc_module_access_leadership_check')+500] and 'No access (hidden)' in (ROOT/'settings.html').read_text() and "[id]==='none'" in (ROOT/'assets/js/app.js').read_text())
...
# V10.4 (pass 52): CBT creation RLS root fix + duration sync + one-file multi-subject import
multi_pg=(ROOT/'cbt-multi.html').read_text()
ok('V10.4 multi-subject RLS gate: sc_cbt_subject_allowed + rebuilt policies',all(x in schema for x in ['sc_cbt_subject_allowed','RUNNING: School Connect CBT multi-subject RLS pack V10.4',"MULTI-SUBJECT:%"]) and schema.count('create or replace function public.sc_cbt_subject_allowed')==1 and 'public.sc_cbt_subject_allowed(auth.uid(),subject,class)' in schema)
ok('V10.4 teacher_can_manage_subject_class staff-array match is case-insensitive','unnest(coalesce(srec.subjects' in schema)
ok('V10.4 createExam: session-backed teacher_id + code-collision retry + RLS translation',all(x in (ROOT/'assets/js/cbt-engine.js').read_text() for x in ['auth.getUser()','duplicate key|cbt_exams_code_key','row-level security|42501','v10.4-cbt-multisubject-rls.sql']))
ok('V10.4 duration coherent everywhere (getter prefers duration_min)','sc_cbt_duration_sync' in schema and 'trg_cbt_duration_sync' in schema and 'duration_min' in (ROOT/'cbt.html').read_text() and 'duration_min' in multi_pg and 'duration_min' in (ROOT/'digital_library.html').read_text())
ok('V10.4 multi-subject one-file import (Section column splitter)',all(x in multi_pg for x in ['importCombined','readCombinedFile','mm-combined','Split into subjects']))
ok('V10.4 DL Send-to-CBT stamps session-backed teacher_id','sb.auth.getUser()' in (ROOT/'digital_library.html').read_text())
# V10.5 (pass 53): Digital Library linked CBT + merged report-card push
dl5=(ROOT/'digital_library.html').read_text()
ok('V10.5 digital_library.cbt_code column + pack banner','add column if not exists cbt_code' in schema and 'RUNNING: School Connect Digital Library CBT link pack V10.5' in schema)
ok('V10.5 DL create/edit forms carry the Linked CBT field with picker + verification',all(x in dl5 for x in ['dl-cbt','dl-edit-cbt','pickCBT','cbtVerify','cbtCode(']))
ok('V10.5 reading card shows Take CBT for linked exams','cbt-exam.html?code='+"'"+'+encodeURIComponent(b.cbt_code)' in dl5 and '🧪 Take CBT' in dl5)
ok('V10.5 Send-to-CBT auto-links the created exam back to the reading',"update({cbt_code:newCode})" in dl5)
ok('V10.5 push panel merges linked-CBT best attempts with quiz scores',all(x in dl5 for x in ['best attempt per student per exam',"neq('cbt_code','')","in('exam_id',ids)",'MERGE LINKED-CBT RESULTS']))
ok('V10.5 cbt_code self-heals on un-upgraded databases',dl5.count('v10.5-dl-cbt-link.sql')>=2)
# V10.6 (pass 54): teacher review, math tools, community feed, notifications, sticky tables, self-sufficient validation
engine6=(ROOT/'assets/js/cbt-engine.js').read_text();manager6=(ROOT/'cbt.html').read_text();crud6=(ROOT/'assets/js/crud.js').read_text();notif6=(ROOT/'assets/js/notifications.js').read_text()
ok('V10.6 teacher review: RPC + shared grader + award-aware regrade',all(x in schema for x in ['cbt_review_result','sc_cbt_grade_result_row','manual_awards','reviewed_by','RUNNING: School Connect CBT review + community pack V10.6']) and schema.count('create or replace function public.cbt_review_result')==1)
ok('V10.6 results modal has the per-question marking panel',all(x in manager6 for x in ['reviewResult','saveReview','data-review-q','awaiting review','cbt_review_result']))
ok('V10.6 math keyboard: focus-tracked insertion + expanded symbol groups',all(x in engine6 for x in ['_lastAnswerField','_trackAnswerFocus','focusin','data-sym','mousedown','Greek','Set & logic']))
ok('V10.6 calculator: safe parser + full scientific set',all(x in engine6 for x in ['calcEval','asin','x!','1/x','DEG','M+','Ans','shunting'.replace('shunting','tokenis')]) and 'eval(d.value' not in engine6)
ok('V10.6 community records default audience=all (Lost & Found feed)','communityModules' in crud6 and "audience='all'" in schema and "module in ('lost_found'" in schema)
...
# V10.9 (pass 57): ledger-truth rendering, fee doctor, lock coverage, world-language read-aloud
crud9=(ROOT/'assets/js/crud.js').read_text();speech9=(ROOT/'assets/js/cbt-speech.js').read_text();fees9=(ROOT/'fees.html').read_text();rc9=(ROOT/'report-cards.html').read_text()
ok('V10.9 ledger-truth modules never resurrect deleted rows','LEDGER_TRUTH' in crud9 and "sessionStorage.removeItem(cacheKey)" in crud9 and 'what you see IS the current official record' in crud9)
ok('V10.9 fee entry guards (non-positive, double-entry, 10x typo)',all(x in crud9 for x in ['POSSIBLE DOUBLE ENTRY','UNUSUALLY LARGE AMOUNT','must be a positive number']) and 'OVERPAYMENT' in crud9)
ok('V10.9 Fee Ledger Doctor: RPC + UI',all(x in schema for x in ['sc_fee_ledger_doctor','RUNNING: School Connect fee-doctor pack V10.9b',"'duplicate'","'unlinked'","'overpayment'","'no_period'",'stale_snapshot']) and 'FeeDoctor' in fees9 and 'fee-doctor-out' in fees9)
ok('V10.9 report lock covers traits/comments/metrics + privacy scoping','RUNNING: School Connect lock-coverage pack V10.9' in schema and schema.count('not public.sc_report_hidden_for')>=8 and "affective_traits.student_id" in schema and "report_comments.student_id" in schema)
ok('V10.9 parent picker removes locked children + generation refuses','lockNotices' in rc9 and 'report_locked' in rc9 and 'Report card unavailable' in rc9)
ok('V10.9 read-aloud: French + 21 languages (scripts + stop-words + voices)',all(x in speech9 for x in ["fr: ['le','la'","fr-FR",'SCRIPTS','Devanagari','Français','Bonjour','détect'.replace('détect','detectLang')]) and 'Kana FIRST' in speech9)
# V11.0 (pass 58): engine 11, analyzer, resume, session fix, renewal-safe keepalive, phonetic TTS, lock race
ttg11=(ROOT/'timetable-generator.html').read_text();speech11=(ROOT/'assets/js/cbt-speech.js').read_text();lic11=(ROOT/'assets/js/license.js').read_text();multi11=(ROOT/'cbt-multi.html').read_text();rc11=(ROOT/'report-cards.html').read_text()
ok('V11.0 engine 11: restricted-first + repair-swap + fill guarantee',all(x in schema for x in ['PHASE 0','PHASE 3','REPAIR-SWAP','RUNNING: School Connect timetable-pro pack V11.0']) and schema.count('create or replace function public.generate_timetable')==1)
ok('V11.0 overload analyzer RPC + wizard button','sc_timetable_capacity' in schema and 'over_capacity' in schema and 'window_too_small' in schema and 'teacher_overloaded' in schema and 'analyze()' in ttg11 and '🩻 Overload Analyzer' in ttg11)
ok('V11.0 wizard resume: full working-state snapshot + honest toast',all(x in ttg11 for x in ['stateKey()','saveState()','Session restored','sc-ttg-state-']))
ok('V11.0 multi-CBT session dropdown sourced from lookups + academic_periods',"from('school_settings')" not in multi11 and "in('kind',['term','session'])" in multi11 and 'is_current' in multi11)
...
ok('V11.6 card BACK side: rules, if-found, blood ribbon, QR verify, signature, barcode',all(x in sup116 for x in ['backHtml','sc-idcard-back','CARD RULES','If found, please return to','SCAN TO VERIFY','barcodeImg']))
...
# V11.7 (pass 65): barcode check-in everywhere, true admission year, auto photos, staff ID access
sup117=(ROOT/'assets/js/super.js').read_text();ic117=(ROOT/'idcards.html').read_text()
ok('V11.7 real Code39 barcode engine (local SVG, no CDN) on card faces',all(x in sup117 for x in ['barcodeSvg','barcodeImg','C39','shape-rendering','barcodeImg(idNo']) and 'repeating-linear-gradient(90deg,#111 0 2px' not in sup117)
ok('V11.7 shared scanner: BarcodeDetector + jsQR fallback + keyboard wedge',all(x in sup117 for x in ['scanner:','BarcodeDetector','code_39','jsQR','attach(onhit']))
ck117=(ROOT/'checkin.html').read_text();cks117=(ROOT/'checkin-staff.html').read_text();at117=(ROOT/'attendance.html').read_text()
ok('V11.7 all three pages scan cards (camera + hardware wedge)',all('Super.scanner' in x and 'Super.scanner.attach' in x for x in [ck117,cks117,at117]))
ok('V11.7 attendance scan-to-mark: register rows flip Present on scan',all(x in at117 for x in ['toggleScan','markScanned','Scan ID cards','marked Present']))
...
# V12.3 (pass 71): students-form photo field restored + WRITE-direction sweep
crud123=(ROOT/'assets/js/crud.js').read_text()
...
# V12.7 (pass 81): archive restore hub — CBT one-click restore + universal hub + scenario-aware login
sch127=(ROOT/'database/complete-schema.sql').read_text()
ok('V12.7 archive-hub.js + cbt-archive.js exist and are node-clean', (ROOT/'assets/js/archive-hub.js').exists() and (ROOT/'assets/js/cbt-archive.js').exists() and 'ArchiveHub' in (ROOT/'assets/js/archive-hub.js').read_text() and 'CBTArchive' in (ROOT/'assets/js/cbt-archive.js').read_text())
ok('V12.7 CBT page: Archive Recovery Center with one-click Restore ALL button (robust, all-inclusive, self-contained, seamless)', 'cbt-archive-hub' in (ROOT/'cbt.html').read_text() and 'Restore ALL Archived Exams' in (ROOT/'cbt.html').read_text() and 'cbt-archive.js' in (ROOT/'cbt.html').read_text() and 'archive-hub.js' in (ROOT/'cbt.html').read_text() and 'Fixes Mistaken Archive' in (ROOT/'cbt.html').read_text())
ok('V12.7 Admin Data + Storage: Universal Archive & Restore Hub', 'universal-archive-hub' in (ROOT/'admin-data.html').read_text() and 'Universal Archive & Restore Hub' in (ROOT/'admin-data.html').read_text() and 'archive-hub.js' in (ROOT/'admin-data.html').read_text() and 'universal-archive-hub-storage' in (ROOT/'storage.html').read_text())
ok('V12.7 login: scenario chooser for parents/students/staff + exact help text + parent/children fields', all(x in (ROOT/'login.html').read_text() for x in ['account_scenario','su-parent-wrap','children_admission_nos','if your school recovered from Drive, enter your admission number — your old results, fees and ID photo will re-link automatically.']) and 'SCLogin.updateScenario' in (ROOT/'login.html').read_text())
ok('V12.7 SQL pack: restore all archived CBT + by filter + student archive restore', (ROOT/'database/v12.7-archive-restore.sql').exists() and all(x in (ROOT/'database/v12.7-archive-restore.sql').read_text() for x in ['sc_restore_all_archived_exams','sc_restore_archived_exams_by_filter','sc_restore_student_archive','sc_list_student_archives','archive restore hub pack V12.7']))
ok('V12.7 complete-schema contains archive restore RPCs', all(x in sch127 for x in ['sc_restore_all_archived_exams','sc_restore_student_archive','archive restore hub pack V12.7']))
ok('V12.7 RUNTIME sync includes archive-hub + cbt-archive + v12.7 SQL', "'assets/js/archive-hub.js'" in (ROOT/'tools/sync-v5-repos.py').read_text() and "'assets/js/cbt-archive.js'" in (ROOT/'tools/sync-v5-repos.py').read_text() and 'v12.7-archive-restore.sql' in (ROOT/'tools/sync-v5-repos.py').read_text())
