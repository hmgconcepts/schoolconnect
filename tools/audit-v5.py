...
ok('V9.6 leadership tier has full access (owner cockpit excepted)','LEADERSHIP_OWNER_ONLY' in (ROOT/'assets/js/app.js').read_text() and 'PRINCIPAL_DENY:new Set([])' in (ROOT/'assets/js/app.js').read_text() and 'HEADTEACHER_DENY:new Set([])' in (ROOT/'assets/js/app.js').read_text() and "['bursar','principal','head_teacher','headteacher'].includes(r))return !App.LEADERSHIP_OWNER_ONLY.has(id)" in (ROOT/'assets/js/app.js').read_text())
