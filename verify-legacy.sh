...
# v7 canonical cumulative schema audit (dependency-free Node test)
echo "[0] v7 single-schema / generated-site audit"
if command -v node >/dev/null 2>&1 && [ -f verify-v7-complete.js ]; then
  node verify-v7-complete.js || exit 1
else
  no "verify-v7-complete.js or node missing"
fi
